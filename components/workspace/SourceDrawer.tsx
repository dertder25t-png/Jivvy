...
export function SourceDrawer({
    ...
    const isOpen = isControlled ? propOpen : !internalCollapsed;
    ...
    // Map existing 'collapsed' usages to local variable for internal logic if needed, 
    // but we primarily use isOpen now. 'collapsed' was !isOpen.
    const collapsed = !isOpen;
    const setCollapsed = (val: boolean) => handleOpenChange(!val);
    ...
    // collapsed state is now mapped to isOpen above, but we need variables available if referenced
    // (Wait, I defined const collapsed = !isOpen; const setCollapsed = ... above)
    // So I don't need to re-declare const [collapsed, setCollapsed] = useState(false);
    ...
    return (
        <ResponsiveDrawer
            ...
            trigger={!hideTrigger && (
                <button
                    className={cn(
                        "flex items-center justify-center bg-surface hover:bg-zinc-900 transition-colors group z-20 h-full w-full"
                    )}
                >
                    {isHorizontal ? (
            ...
            className={isHorizontal ? "h-[60vh]" : "w-[400px]"}
            title={activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}
        >
            <div className={cn(
                "flex bg-surface border-zinc-800 overflow-hidden relative transition-all duration-300 flex-col h-full",
                // Remove width/height constraints as they are handled by drawer
                "w-full h-full",
                className
            )}>
                {/* Header with Tabs */}
